...
void load_css(t_master *m)
{
	...
	proj_path(init_str("variables" SLASH_S "settings.bin", &path_end), m);
	...
	if (global_settings)
	{
		...
		{
			m->dark_mode = c;
			proj_path(init_str((c ? "variables" SLASH_S "darkstyle.css" : "variables" SLASH_S "lightstyle.css"), &path_end), m);
